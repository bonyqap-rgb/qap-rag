...
test("API POST /documents/upload - uploads valid PDF successfully", async () => {
  ...
  documentService.createDocument = async (payload: any) => ({
    ...mockDoc,
    ...payload,
    id: "new-uploaded-uuid"
  });

  const formData = new FormData();
  const blob = new Blob(["%PDF-1.4 dummy pdf content"], { type: "application/pdf" });
  formData.append("file", blob, "test_manual.pdf");
  formData.append("title", "Manual de Teste");
  ...
  try {
    ...
    assert.strictEqual(res.status, 201);
    const body = await res.json() as Document;
    assert.strictEqual(body.id, "new-uploaded-uuid");
    assert.strictEqual(body.title, "Manual de Teste");
    assert.strictEqual(body.processingStatus, "pending");
  } finally {
    documentService.createDocument = originalCreate;
  }
});

test("API POST /documents/upload - returns 400 when file is missing", async () => {
  const formData = new FormData();
  ...
  const body = await res.json() as any;
  assert.strictEqual(body.error, "ERROR");
  assert.strictEqual(body.message.includes("Nenhum arquivo enviado"), true);
});

test("API POST /documents/upload - returns 400 when file is empty", async () => {
  const formData = new FormData();
  ...
  const body = await res.json() as any;
  assert.strictEqual(body.error, "ERROR");
  assert.strictEqual(body.message.includes("enviado está vazio"), true);
});

test("API POST /documents/upload - returns 400 when file is not PDF", async () => {
  const formData = new FormData();
  const blob = new Blob(["some plain text"], { type: "text/plain" });
  ...
  const body = await res.json() as any;
  assert.strictEqual(body.error, "ERROR");
  assert.strictEqual(body.message.includes("Apenas arquivos PDF são permitidos"), true);
});

test("API POST /documents/upload - returns 400 when file is oversized", async () => {
  const formData = new FormData();
  const blob = new Blob([new Uint8Array(51 * 1024 * 1024)], { type: "application/pdf" });
  ...
  const body = await res.json() as any;
  assert.strictEqual(body.error, "ERROR");
  assert.strictEqual(body.message.includes("limite de tamanho de 50 MB"), true);
});
