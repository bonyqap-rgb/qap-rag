...
test("SearchService - search with category filter", async () => {
  const originalFrom = supabase.from;
  const originalRpc = supabase.rpc;

  let fromCalledWith: string[] = [];
  ...
  supabase.from = function (table: string) {
    ...
    const queryBuilder = {
      ...
      then: (resolve: any) => {
        if (table === "documents") {
          resolve({ data: [{ filename: "manual.pdf" }], error: null });
  ...
  let rpcFilteredIds: any[] | null = null;
    ...
    const queryBuilder = {
      eq: (col: string, val: string) => {
        ...
        return Promise.resolve({
          data: [
            {
              document_id: "k-doc-uuid",
              chunk_index: 0,
              content: "Trecho filtrado por categoria",
              similarity: 0.9,
            },
          ],
          error: null,
  ...
  try {
    ...
    assert.ok(fromCalledWith.includes("documents"));
    assert.ok(fromCalledWith.includes("knowledge_documents"));
    assert.deepStrictEqual(rpcFilteredIds, ["k-doc-uuid"]);
    assert.strictEqual(results.length, 1);
    assert.strictEqual(results[0].documentId, "k-doc-uuid");
  } finally {
...
test("SearchService - search handles early-exit on non-existent category/type", async () => {
  const originalFrom = supabase.from;
  const originalRpc = supabase.rpc;
  ...
  try {
    ...
    assert.strictEqual(results.length, 0);
    assert.strictEqual(rpcCalled, false); // Optimized exit: should not call pgvector rpc!
  } finally {
    supabase.from = originalFrom;
    supabase.rpc = originalRpc;
  }
