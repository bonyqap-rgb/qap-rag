...
export class IndexerService {
  ...
  async indexDocument(documentId: string): Promise<void> {
    ...
    try {
      // 1. Fetch document (throws NotFoundError if not found, validating existence)
      const doc = await this.documentService.getDocumentById(documentId);
      documentTitle = doc.title;

      // Validate document state before indexing
      if (doc.processingStatus === "processing") {
        throw new Error(`O documento com ID '${documentId}' já está em processamento.`);
      }

      // Validate that document has extracted text to process
      ...
      // 5. Store chunk vectors in Supabase pgvector (Transactional and Idempotent)
      ...
      await this.vectorService.saveChunksTransactional(documentId, chunksToSave);
      const vectorDuration = Date.now() - vectorStart;

      logger.info(`[INDEXER] Salvamento vetorial transacional concluído (idempotente). ID: ${documentId}, Tempo: ${vectorDuration}ms`);
