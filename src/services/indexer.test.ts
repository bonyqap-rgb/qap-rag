...
test("IndexerService - successfully executes complete indexing pipeline", async () => {
  // Setup mocks
  ...
  // Verify that chunks were saved properly with accurate indices and embeddings
  assert.strictEqual(savedChunks.length, 2);
  assert.strictEqual(savedChunks[0].chunkIndex, 0);
  assert.strictEqual(savedChunks[0].texto.includes("Esta é a página um."), true);
  assert.strictEqual(savedChunks[0].embedding.length, 768);
});
