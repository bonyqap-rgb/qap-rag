...
export class VectorService {
  ...
  /**
   * Batch saves chunks and their embeddings to the database transactionally and idempotently.
   * Invokes the PL/pgSQL save_document_chunks_json RPC function.
   */
  async saveChunksTransactional(documentId: string, chunks: Omit<DocumentChunk, "id" | "createdAt">[]): Promise<void> {
    if (chunks.length === 0) return;

    const payload = chunks.map((c) => ({
      chunk_index: c.chunkIndex,
      texto: c.texto,
      // Convert number array to pgvector string format: "[v1,v2,v3,...]"
      embedding: `[${c.embedding.join(",")}]`,
    }));

    const { error } = await this.supabase.rpc("save_document_chunks_json", {
      p_document_id: documentId,
      p_chunks: payload,
    });

    if (error) {
      throw new Error(`Erro transacional ao salvar chunks no banco vetorial: ${error.message}`);
    }
  }
